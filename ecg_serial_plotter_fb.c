...
const unsigned char font_data[96][12] = {
    ...
    {0x3C,0x66,0x06,0x06,0x06,0x06,0x06,0x06,0x66,0x3C,0x00,0x00}, // 'C' (67)
...
int init_framebuffer() {
    ...
    if (fb_fd == -1) {
        printf("无法打开 /dev/fb1，尝试 /dev/fb0\n");
        fb_fd = open("/dev/fb0", O_RDWR);
        if (fb_fd == -1) {
            printf("错误：无法打开framebuffer设备\n");
            return -1;
        }
    }
    
    if (ioctl(fb_fd, FBIOGET_FSCREENINFO, &finfo)) {
        printf("错误：无法获取framebuffer固定信息\n");
        close(fb_fd);
        return -1;
    }
    
    if (ioctl(fb_fd, FBIOGET_VSCREENINFO, &vinfo)) {
        printf("错误：无法获取framebuffer可变信息\n");
        close(fb_fd);
        return -1;
    }
    
    printf("FrameBuffer信息: %dx%d, bpp: %d\n", vinfo.xres, vinfo.yres, vinfo.bits_per_pixel);
    
    if (vinfo.bits_per_pixel != 16) {
        printf("警告：framebuffer不是16位色深，实际: %dbpp\n", vinfo.bits_per_pixel);
    }
